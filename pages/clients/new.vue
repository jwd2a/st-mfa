              ...
              <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                  ...
                  <div class="mt-1 flex space-x-2 items-center">
                    ...
                    <div class="flex items-center whitespace-nowrap">
                      <input 
                        ...
                        type="checkbox" 
                        class="focus:ring-primary h-4 w-4 text-primary border-gray-300 dark:border-gray-600 rounded mr-1.5"
                        @change="handlePrimaryChange(index)"
                      />
                      <label :for="`primary-${index}`" class="text-xs font-medium text-gray-700 dark:text-gray-300">Primary</label>
                    </div>
                  </div>
                </div>
                ...
                <div class="sm:col-span-6">
                  ...
                  <p class="mt-1 text-xs text-gray-500 dark:text-gray-400 mb-2">
                    Select the services associated with this phone number
                  </p>
                  ...
                  <div class="relative max-w-md">
                    <div class="flex">
                      <div class="relative flex-grow">
                        ...
                        <input 
                          ...
                          class="pl-10 shadow-sm focus:ring-primary focus:border-primary block w-full sm:text-sm border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100 rounded-md"
                        />
                      </div>
                    </div>
                    ...
                    <!-- Service dropdown -->
                    <div 
                      v-show="activeServiceDropdown === index"
                    ...
                    >
                      <div v-if="filteredServices(index).length === 0" class="px-4 py-2 text-sm text-gray-500 dark:text-gray-400">
                      ...
                      <div 
                        ...
                        class="cursor-pointer select-none py-2 pl-3 pr-9 hover:bg-gray-100 dark:hover:bg-gray-700"
                        :class="{ 'bg-gray-100 dark:bg-gray-700': phone.selectedServices.includes(service.id) }"
                      >
                        <div class="flex items-center">
                          <span class="text-sm text-gray-900 dark:text-gray-100">{{ service.name }}</span>
                          <Icon 
                            v-if="phone.selectedServices.includes(service.id)" 
                            name="lucide:check" 
                  ...
                  <div class="flex flex-wrap gap-2 mt-3">
                    ...
                    >
                      ...
                      <button 
                        type="button" 
                        class="ml-1.5 h-3.5 w-3.5 rounded-full flex items-center justify-center text-blue-400 hover:bg-blue-200 hover:text-blue-600"
                        @click="removeService(index, serviceId)"
                      >
                        <span class="sr-only">Remove service</span>
                        <Icon name="lucide:x" class="h-2.5 w-2.5" />
                      </button>
                    </div>
                    <div v-if="phone.selectedServices.length === 0" class="text-xs text-gray-500 dark:text-gray-400 italic">
...
// Handle primary checkbox (only one can be selected)
const handlePrimaryChange = (selectedIndex: number) => {
  formData.value.phoneNumbers.forEach((phone, index) => {
...
const addPhoneNumber = () => {
  formData.value.phoneNumbers.push({
    ...
    label: '',
    selectedServices: [],
    isPrimary: formData.value.phoneNumbers.length === 0
  });
...
const removePhoneNumber = (index: number) => {
  const isRemovingPrimary = formData.value.phoneNumbers[index].isPrimary;
  formData.value.phoneNumbers.splice(index, 1);
  serviceSearchQueries.value.splice(index, 1);
  
  // If we removed the primary number and there are still others, make the first one primary
  if (isRemovingPrimary && formData.value.phoneNumbers.length > 0) {
    formData.value.phoneNumbers[0].isPrimary = true;
  }
};
